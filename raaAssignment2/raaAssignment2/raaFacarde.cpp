...
void raaFacarde::toggleNames()
{
	...
	for (raaFacardes::iterator it = sm_lFacardes.begin(); it != sm_lFacardes.end(); it++)
		(*it)->showName(sm_bShowNames);
}
...
void raaFacarde::init()
{
	if (m_pRoot)
	{
		...
		// Text of car was inside it, was the same as the example but now its clearer to see car names
		if (m_pRoot->getName().find("car") != std::string::npos)
